...
import os
from apps.vadmin.system import models as auth_models
...
class InitializeData:
    ...
    def migrate_model(cls, env: Environment = Environment.pro):
        ...
        模型迁移映射到数据库
        """
        # 先升级数据库到最新版本，确保数据库是最新的（避免 "Target database is not up to date" 错误）
        try:
            subprocess.check_call(['alembic', '--name', f'{env.value}', 'upgrade', 'head'], cwd=BASE_DIR)
        except subprocess.CalledProcessError:
            # 如果升级失败，可能是数据库已经是最新版本，继续执行
            pass
        ...
        # 应用新生成的迁移
        subprocess.check_call(['alembic', '--name', f'{env.value}', 'upgrade', 'head'], cwd=BASE_DIR)
        print(f"环境：{env.value}  {VERSION} 数据库表迁移完成")
