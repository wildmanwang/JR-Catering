    ...
    def generate_code(self) -> str:
        ...
        for attr_name, column_property in mapper.column_attrs.items():
            if attr_name in self.BASE_FIELDS:
                continue
            # 检查是否有列，并且是真正的 Column 对象（不是 column_property 计算属性）
            if not column_property.columns:
                continue
            column = column_property.columns[0]
            # 检查是否是真正的 Column 对象（有 nullable 属性）
            if not hasattr(column, 'nullable'):
                # 跳过 column_property 计算属性（如 Label 对象）
                continue
            # 确保是 Column 类型
        ...
        # 添加需要的导入
        if "decimal" in required_imports:
