...
def init_app(path: str):
    ...
    :param path: app 路径，根目录为apps，填写apps后面路径即可，例子：vadmin/system
    """
...
if __name__ == '__main__':
    ...
    # 1. 进入后台目录
    # 2. 进入虚拟环境
    # 3. uvicorn main:create_app --host 0.0.0.0 --port 9000 --reload

    # 终端：数据库迁移
    # 1. 数据模型编辑完成
    # 2. 在alembic\env.py文件中导入需要迁移的模型
    # 3. G:\Cliff\anaconda3\envs\jrcatering\python.exe main.py migrate --env dev

    # 运行：生成crud代码
    # from scripts.crud_generate.main import CrudGenerate
    # from apps.vadmin.product.models.combo import DishComboSeries, DishCombo, DishComboDetail
    # crud = CrudGenerate(DishComboDetail , "菜品套餐详情", "dish_combo_detail")
    # 只打印代码，不执行创建写入
    # crud.generate_codes()
    # 创建并写入代码
    # crud.main()
