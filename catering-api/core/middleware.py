...
from application.settings import OPERATION_RECORD_METHOD, OPERATION_LOG_RECORD, IGNORE_OPERATION_FUNCTION, \
...
def register_operation_record_middleware(app: FastAPI):
    ...
    async def operation_record_middleware(request: Request, call_next):
        ...
        # 在中间件中创建数据库会话，记录操作日志
        # 使用 try-except 确保日志记录失败不影响主请求
        try:
            async with session_factory() as db:
                async with db.begin():
                    await OperationRecordDal(db).create_data(data)
