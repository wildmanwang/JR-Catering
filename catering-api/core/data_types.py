...
def decimal_vali(value: Decimal | float | int | str | None):
    """
    Decimal 验证和转换
    支持 Decimal、float、int、str 类型输入，统一转换为 Decimal
    """
    if value is None:
        return None
    ...
    if isinstance(value, str):
        try:
            ...
            raise ValueError("无效的 Decimal 数据")
    raise ValueError("无效的 Decimal 数据")


# 使用 decimal_vali 验证并序列化为 float 的 Decimal 类型
DecimalType = Annotated[
    Decimal | float | int | str | None,
    AfterValidator(decimal_vali),
