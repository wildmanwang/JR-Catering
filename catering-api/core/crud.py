...
class DalBase:
    ...
    def __dict_filter(self, **kwargs) -> list[BinaryExpression]:
        ...
        """
        conditions = []
        fuzzy_query_str = kwargs.pop('fuzzy_query_str', None)
        like_fields = []
        
        # 第一遍遍历：收集所有 like 字段
        for field, value in kwargs.items():
            if value is not None and value != "":
                if isinstance(value, tuple) and len(value) == 2 and value[0] == "like":
                    try:
                        attr = getattr(self.model, field)
                        like_fields.append(attr)
                ...
                if isinstance(value, tuple):
                    if len(value) == 1:
                        ...
                        if value[0] == "date":
                            ...
                            # 如果存在模糊查询字符串，则不单独处理 like 字段，统一在最后处理
                            if not fuzzy_query_str:
        ...
        # 如果存在模糊查询字符串且有 like 字段，构建 OR 查询条件
        if fuzzy_query_str and like_fields:
            from sqlalchemy import or_
            fuzzy_value = f"%{fuzzy_query_str}%"
            or_conditions = [field.like(fuzzy_value) for field in like_fields]
            if or_conditions:
