<script setup lang="tsx">
          ...
          default: (data: any) => {
            ...
            return (
              <div class="resource-image-name flex items-center">
                <div>
                  <ElImage
                    src={hasImage ? `${row.dish_images[0]}?x-oss-process=image/resize,m_fixed,h_100` : DEFAULT_IMAGE}
                    zoom-rate={1.2}
    ...
    const handleImageUpload = async (formData: any) => {
      if (!formData.dish_images || !Array.isArray(formData.dish_images)) return
      
      for (let index = 0; index < formData.dish_images.length; index++) {
        const fileData = formData.dish_images[index]
     ...
     */
    watch(
      [() => dataList.value, () => currentPage.value, () => pageSize.value, () => activeIndex.value, () => curKitchen.value],
      async () => {
    ...
    <template>
      <ContentWrap v-show="pageReady">
        ...
        <div class="content-area">
          <el-card
            class="left-list" header="厨部" shadow="never">
            <el-menu
              header="厨部"
              class="left-list"
              shadow="never"
            ...
            >
              ...
              >
                {{ kitchen.name_unique }}<span v-if="kitchen.dish_count > 0">[{{ kitchen.dish_count }}]</span>
              </el-menu-item>
            </el-menu>
          </el-card>
