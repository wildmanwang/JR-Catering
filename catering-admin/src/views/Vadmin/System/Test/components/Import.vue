<script setup lang="ts">
    ...
    import { addDishListApi, putDishListApi, getDishStatusOptionsApi } from '@/api/vadmin/product/dish'
    ...
    const loadOptions = async () => {
      ...
      if (colStatus) {
        ...
        colStatus.options = res.data.map(item => ({
          value: item.id,
          label: item.name
        }))
      }
    
      const colKitchen = tableColumns.find(col => col.field === 'kitchen_id')
      if (colKitchen) {
        const res = await getKitchenListApi({is_active: true})
        colKitchen.options = res.data.map(item => ({
          value: item.id,
          label: item.name
        }))
      }
