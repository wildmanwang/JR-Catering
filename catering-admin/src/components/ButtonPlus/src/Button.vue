<script setup lang="ts">
    ...
    const props = defineProps({
      stype: {
        ...
        required: true,
        validator: (value) => {
            const validValues = ['new', 'newcontinue', 'newcopy', 'modify', 'delete', 'save', 'copy', 'batch', 'query', 'select', 'refresh', 'ok', 'cancel', 'import', 'export', 'print', 'setting', 'return', 'normal']
            const isValid = validValues.includes(value.toLowerCase())
            if (!isValid) {
              throw new Error(`ButtonPlus组件stype属性值${value}无效，应取以下值之一：${validValues.join(',')}`)
            }
            return true
        }
      },
    ...
    const btnMap = {
        ...
        "newcopy": ["拷貝新增", new URL("@/assets/svgs/btn/btn-new.svg", import.meta.url).href, 'rgb(99, 161, 3)', 'rgb(255, 255, 255)', '1px solid rgb(99, 161, 3)'],
