  ...
  setup(props, { emit }) {
    ...
    const handleCheckAllChange = (val: boolean) => {
      tableColumns.value.forEach((item) => {
        // showConfig 为 true 时，可配置显示/隐藏
        if (item.showConfig !== false) {
          item.show = val
        }
      })
      isIndeterminate.value = tableColumns.value
        .filter((item) => item.showConfig !== false)
        .some((item) => item.show)
    }
    ...
    if (cacheTableHeadersKey) {
      ...
      if (cacheData) {
        tableColumns.value.forEach((item) => {
          ...
          // 只有当缓存中存在该字段的数据时才更新
          if (fieldData) {
            if (fieldData.index !== undefined) {
              item._index = fieldData.index
            }
            if (fieldData.show !== undefined) {
        ...
        // 只有当所有列都有 _index 时才排序
        const allHaveIndex = tableColumns.value.every((item) => item._index !== undefined)
        if (allHaveIndex) {
          tableColumns.value.sort((a, b) => (a._index || 0) - (b._index || 0))
        }
                              ...
                              <div class="flex justify-between">
                                <div>
                                  ...
                                  <ElCheckbox
                                    ...
                                    disabled={element.showConfig === false}
